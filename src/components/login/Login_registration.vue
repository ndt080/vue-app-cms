<template>
  <form class="form card-deck" action='' method="POST"
      ...
      metaInfo:{
        ...
        methods: {
            async submitHandler (){
                ...
                try{
                    this.$toast.success('Учетная запись создана! Теперь можно авторизироваться!');
                    await this.$store.dispatch('register', formData)
                    await this.$router.push('/')
                }catch (e) {
                  this.$toast.error('Ошибка регистрации. Проверьте вводимые данные!');
                }
            },
        }
    }
